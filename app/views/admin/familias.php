    ...
    <div id="main-content-wrap" class="flex-1 flex flex-col min-h-screen w-full overflow-x-hidden" style="margin-left:16rem">
        ...
        <main class="flex-1 p-4 md:p-8 max-w-[1280px] mx-auto w-full flex flex-col gap-6">
            <div class="flex flex-wrap justify-between items-center gap-4">
                <div>
                    <h2 class="text-headline-lg text-secondary font-headline-lg flex items-center gap-2">
                        <span class="material-symbols-outlined text-3xl">family_restroom</span>
                        Familias / Acudientes (<?php echo count($data['familias']); ?>)
                    </h2>
                    <p class="text-on-surface-variant text-sm mt-1">Listado de acudientes y familias vinculadas a la comunidad educativa.</p>
                </div>
            </div>
                ...
                <table class="w-full text-left border-collapse text-sm">
                    <thead>
                        <tr class="bg-surface-container/60 border-b border-outline-variant">
                            <th class="px-4 py-3.5 text-on-surface font-semibold">Acudiente Principal</th>
                            ...
                            <th class="px-4 py-3.5 text-on-surface font-semibold">Estudiantes</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if (empty($data['familias'])): ?>
                            <tr><td colspan="6" class="px-4 py-8 text-center text-on-surface-variant">No hay familias registradas.</td></tr>
                        <?php else: ?>
                            ...
                            <tr class="border-b border-outline-variant/50 hover:bg-surface-container/40 transition-colors">
                                ...
                                <td class="px-4 py-3.5 text-on-surface-variant font-mono text-xs"><?php echo htmlspecialchars($fam->documento_principal_acudiente); ?></td>
                                <td class="px-4 py-3.5 text-on-surface-variant"><?php echo htmlspecialchars($fam->email_contacto); ?></td>
                                <td class="px-4 py-3.5 text-on-surface-variant"><?php echo htmlspecialchars($fam->telefono_contacto ?? '—'); ?></td>
                                <td class="px-4 py-3.5">
                                    <code class="bg-secondary/10 text-secondary px-2 py-0.5 rounded text-xs font-semibold"><?php echo htmlspecialchars($fam->username); ?></code>
                                </td>
                                <td class="px-4 py-3.5 text-on-surface-variant text-xs"><?php echo htmlspecialchars($fam->estudiantes ?? '—'); ?></td>
                            </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </tbody>
