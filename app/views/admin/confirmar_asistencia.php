...
<div class="flex">
    ...
    <main id="mainContent" class="flex-1 min-h-screen bg-surface-container-lowest flex flex-col">
        ...
        <!-- Main Content -->
        <main class="flex-1 p-4 md:p-8 max-w-4xl mx-auto w-full flex flex-col gap-6">
            ...
            <!-- Scanner Area -->
            <div id="scannerContainer" class="bg-surface border border-outline-variant rounded-xl p-6 shadow-sm hidden">
...
function showCameraPermissionModal() {
    return new Promise((resolve) => {
        ...
        modal.innerHTML = `
            ...
            <div class="relative bg-surface rounded-3xl shadow-2xl border border-outline-variant/50 w-full max-w-md transform transition-all duration-300 scale-95 opacity-0" id="cameraModalContent">
...
async function initScanner() {
    ...
    // Stop previous ZXing scanner if exists
    if (codeReader) {
        try {
    ...
    // Stop native camera if active
    if (CameraController.isActive()) {
        CameraController.stop();
        isUsingNativeCamera = false;
    }

    // Create new ZXing BrowserQRCodeReader
    codeReader = new ZXing.BrowserQRCodeReader();

    try {
        // Get available cameras via ZXing (for device enumeration)
...
// Start native camera and then ZXing scanning
async function startCameraAndScan() {
    ...
    try {
        ...
        // Now start ZXing decoding on the video element
...
function onScanSuccess(decodedText) {
    ...
    fetch('<?php echo URLROOT; ?>/admin/procesar_qr_asistencia', {
        method: 'POST',
        headers: {
