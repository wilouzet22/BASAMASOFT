            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-on-surface-variant">
                        Admin: <span class="text-primary font-bold"><?php echo htmlspecialchars($_SESSION['username'] ?? ''); ?></span>
                    </p>
                </div>
            </div>
            <div class="flex items-center gap-6">
                ...
                <div class="flex items-center gap-3 pl-4 border-l border-outline-variant">
                    <div class="text-right">
                        ...
                        <p class="text-[10px] text-outline uppercase font-bold tracking-tighter">Panel Admin</p>
                    </div>
                    <a href="<?php echo URLROOT; ?>/auth/logout" onclick="event.preventDefault(); openLogoutModal();"
                       class="w-10 h-10 rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center overflow-hidden hover:bg-primary/20 transition-all cursor-pointer shadow-sm" title="Cerrar sesión">
...
async function generarQr(forzar = false) {
    ...
    try {
        const res  = await fetch(`${URLROOT}/admin/generar_qr_actividad/${id}`);
        ...
        if (data.renovado || forzar) countdownSeconds = 300;
...
function renderQR(url, data) {
    ...
    document.getElementById('qrActividadNombre').textContent = sel.options[sel.selectedIndex].text;
    document.getElementById('qrExpiraEn').textContent = 'Expira: ' + new Date(data.expira_en).toLocaleTimeString('es-ES', {hour:'2-digit', minute:'2-digit'});
}
...
function startCountdown() {
    ...
    countdownTimer = setInterval(() => {
        ...
        if (countdownSeconds <= 0) { clearInterval(countdownTimer); generarQr(true); }
    }, 1000);
}
...
async function fetchCounter() {
    ...
    try {
        const res  = await fetch(`${URLROOT}/admin/qr_actividad_status/${currentActividadId}`);
        const data = await res.json();
